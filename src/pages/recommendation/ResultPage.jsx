...
const ResultPage = () => {
  ...
  const fetchRecommendations = useCallback(async () => {
    try {
      ...
      // localStorage에서 추천 결과 가져오기 (SetLocationPage에서 저장한 결과)
      const recommendationResult = localStorage.getItem("recommendationResult");

      if (recommendationResult) {
        // 이미 추천 결과가 있는 경우
        const result = JSON.parse(recommendationResult);
        console.log("저장된 추천 결과:", result);

        if (result.success) {
          const recommendationsData = result.data.recommendations || [];
          console.log("받은 추천 데이터:", recommendationsData);
          console.log("추천 데이터 길이:", recommendationsData.length);
          ...
          console.error("저장된 추천 결과에 오류:", result.error);
          setError("추천 요청에 실패했습니다: " + result.error);
        }
      } else {
        // 추천 결과가 없는 경우, 히스토리 조회로 fallback
        console.log("저장된 추천 결과가 없습니다. 히스토리 조회로 fallback");
                    ...
                    <div className="card-content">
                      ...
                      <div className="card-scores">
                        ...
                        <span className="final-score">
                          점수:{" "}
                          {recommendations[currentIndex]?.finalScore || "N/A"}
                        </span>
