...
const SetThemePage = () => {
  ...
  const loadCategory = async () => {
    try {
      ...
      const placeCategory =
        categoryList.find((item) => item.name === "장소") || {};
  ...
  const handleKeywordSelect = async (label) => {
    ...
    if (newValue && newValue !== "상관없음") {
      try {
        ...
        if (matched?._id) {
          localStorage.setItem("selectedThemeKeywordId", matched._id);
          localStorage.setItem("selectedThemeKeyword", newValue);
          console.log(
            "저장된 테마 키워드 ID:",
            matched._id,
            "키워드:",
