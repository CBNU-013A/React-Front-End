...
const SetSeasonPage = () => {
  ...
  const [previousSelectedActivity, setPreviousSelectedActivity] = useState("");
  ...
  const loadPreviousSelection = () => {
    ...
    const activity = localStorage.getItem("selectedActivityKeyword") || "";
  ...
  const loadCategory = async () => {
    try {
      ...
      const placeCategory =
        categoryList.find((item) => item.name === "계절 / 시간대") || {};
  ...
  const loadKeywords = async (categoryId) => {
    try {
      ...
      const filtered = allKeywords
        ...
        .map((item) => item.name)
        .filter((keyword) => keyword !== "낮" && keyword !== "야간"); // 낮, 야간 제외
  ...
  const handleKeywordSelect = async (label) => {
    ...
    if (newValue && newValue !== "상관없음") {
      try {
        ...
        if (matched?._id) {
          localStorage.setItem("selectedSeasonKeywordId", matched._id);
          localStorage.setItem("selectedSeasonKeyword", newValue);
          console.log(
            "저장된 계절 키워드 ID:",
            matched._id,
            "키워드:",
  ...
  const handleActivityClick = () => {
    navigate("/recommendation/activity");
  };
        ...
        <div className="previous-selections">
          ...
          <div className="selection-item">
            ...
            {previousSelectedActivity && (
              <button className="selection-tag" onClick={handleActivityClick}>
                {previousSelectedActivity}
              </button>
            )}
          </div>
